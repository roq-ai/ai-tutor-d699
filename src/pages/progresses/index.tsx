...
import { useRoqClient, useProgressFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ProgressInterface, unknown>;
...
export function ProgressListPage(props: ProgressListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useProgressFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.progress.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ProgressInterface) => {
    if (hasAccess('progress', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/progresses/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('module', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'module',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/modules/view/${record.module?.id}`}>
              {record.module?.name}
            </Link>
          ),
        }
