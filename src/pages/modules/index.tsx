...
import { useRoqClient, useModuleFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ModuleInterface, unknown>;
...
export function ModuleListPage(props: ModuleListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['content', 'progress.count', 'quiz.count'],
  ...
  const { data, error, isLoading, mutate } = useModuleFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.module.delete({ where: { id } });
      await mutate();
    } catch (error) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/modules/view/${record.id}`} passHref legacyBehavior>
                <Button
