...
import { useRoqClient, useModuleFindFirst } from 'lib/roq';
...
function ModuleEditPage() {
  ...
  const { data, error, isLoading, mutate } = useModuleFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ModuleInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.module.update({
        data: values as RoqTypes.module,
        where: {
          id,
        },
      });
