...
import { useRoqClient, useQuizFindFirst } from 'lib/roq';
...
function QuizEditPage() {
  ...
  const { data, error, isLoading, mutate } = useQuizFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: QuizInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.quiz.update({
        data: values as RoqTypes.quiz,
        where: {
          id,
        },
      });
