...
import { useRoqPlatformUserProfiles, useModuleCount, useQuizCount, useProgressCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const moduleStat = useModuleCount();

  const quizStat = useQuizCount();

  const progressStat = useProgressCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Modules',
      result: moduleStat?.data,
      isLoading: moduleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Quizzes',
      result: quizStat?.data,
      isLoading: quizStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Progresses',
      result: progressStat?.data,
